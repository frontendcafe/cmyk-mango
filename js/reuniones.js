...
selectElement.addEventListener('change', async (event) => {
  try {
    ...
    const listOfMeetings = await getMeetings({
    ...
    if (listOfMeetings.meetings.length > 0) {
      listOfMeetings.meetings.forEach((meeting) => {
        concatenatedMeetings =
          concatenatedMeetings +
          /*html*/ `<div class="meeting-item" id="meeting-item">
                    <p class="meeting-item-date">${
                      meeting.datetime !== null &&
                      meeting.datetime !== undefined
                        ? meeting.datetime.length > 0
                          ? meeting.datetime[[meeting.datetime.length - 1]]
                              .toDate()
                              .toDateString()
                          : ''
                        : ''
                            ...
                            ${
                              ...
                              meeting.participants !== undefined
                                ? meeting.participants.length > 0
                                  ? `<p class="votes">${meeting.participants.length} <br>votos</p>`
                                  : `<a id="vote" href=""><img src="../assets/img/check.svg" alt=""></a>`
                                : `<a id="vote" href=""><img src="../assets/img/check.svg" alt=""></a>`
                            }
                            <a id="more" href="./reunion.html?id=${
                              meeting.id
                            }"><img src="../assets/img/arrow.svg" alt=""></a>
                        </div>
                    </div>
                </div>`;
      });
